...
async function connect() {
  ...
  //
  // FIX 3: Guard against re-processing non-retry updates (delivery receipts,
  // read status changes, reactions, edits). Only route through handleMessage
  // when the update contains real message content that isn't pure protocol noise.
  sock.ev.on('messages.update', async (updates) => {
    for (const { key, update } of updates) {
      if (!update.message) continue;
      const msgKeys = Object.keys(update.message);
      const isProtocol = msgKeys.every(k =>
        ['messageContextInfo', 'senderKeyDistributionMessage',
         'protocolMessage', 'reactionMessage'].includes(k)
      );
